...
- Firmware must reuse an active in-memory `device_code` and must not create another pending code for the same device until the current code is cleared, approved, expired, or lost after restart.
